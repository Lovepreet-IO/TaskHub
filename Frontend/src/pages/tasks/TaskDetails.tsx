...
import {showSuccess, showError, showLoading} from "../../components/common/toast.ts";
...
const TaskDetails = () => {
    ...
    const fetchComments = async () => {
        try {
            ...
            // Some API responses wrap the comment in `data.Comment`
            const commentData = res.data?.data?.Comment;
            if (commentData) {
                setComments([commentData]); // If API returns single comment
            } else {
                setComments([]);
            }
        } catch (err) {
